...
echo -e "${BLUE}==========================================================${PLAIN}"
echo -e "${BLUE}        欢迎使用 NimbusVPN 一键源码部署与管理脚本${PLAIN}"
echo -e "${BLUE}==========================================================${PLAIN}"

# 3. Configure GitHub Repository URL
# Default to the official repository (nimbus-vpngate)
DEFAULT_USER="baoweise-bot"
DEFAULT_REPO="nimbus-vpngate"
...
if command -v systemctl >/dev/null 2>&1; then
    echo -e "  -> 检测到 systemd，正在创建服务配置 /lib/systemd/system/nimbusvpn.service ..."
    cat > /lib/systemd/system/nimbusvpn.service <<EOF
[Unit]
Description=NimbusVPN OpenVPN Manager with HTTP/SOCKS5 Proxy
...
Restart=always
RestartSec=5
EnvironmentFile=-/etc/default/nimbusvpn

[Install]
WantedBy=multi-user.target
EOF
    systemctl daemon-reload
    systemctl enable nimbusvpn.service
elif command -v rc-service >/dev/null 2>&1; then
    echo -e "  -> 检测到 OpenRC，正在创建服务配置 /etc/init.d/nimbusvpn ..."
    cat > /etc/init.d/nimbusvpn <<EOF
#!/sbin/openrc-run
...
EOF
    chmod +x /etc/init.d/nimbusvpn
    rc-update add nimbusvpn default
else
...
INSTALL_DIR = "/opt/nimbusvpn"
LOG_FILE = "/opt/nimbusvpn/vpngate_data/vpngate.log"
...
def load_ui_cfg():
    import json
    path = "/opt/nimbusvpn/vpngate_data/ui_auth.json"
...
def load_state():
    import json
    path = "/opt/nimbusvpn/vpngate_data/state.json"
...
def get_active_node_info():
    import json
    path = "/opt/nimbusvpn/vpngate_data/nodes.json"
    state = load_state()
...
def get_public_ip():
    path = "/opt/nimbusvpn/vpngate_data/public_ip.txt"
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                ip = f.read().strip()
...
def get_service_pid(service_name="nimbusvpn.service"):
    try:
...
def check_service_active(service_name="nimbusvpn.service"):
            ...
            if pid_dir.isdigit():
                try:
                    with open(os.path.join('/proc', pid_dir, 'cmdline'), 'r') as f:
                        ...
                        if 'openvpn' in cmd and ('/opt/nimbusvpn/vpngate_data' in cmd or '/opt/nimbusvpn/vpngate_data/configs' in cmd):
                            return True
                except Exception:
                    continue
    except Exception:
...
def print_status():
    ...
    service_ok = check_service_active("nimbusvpn.service")
    openvpn_ok = check_openvpn_process()
    pid = get_service_pid("nimbusvpn.service")
    ...
    print_line("=======================================================")
    print_line(f"               {bold}NimbusVPN 管理终端 v2.0{reset}                  ")
    print_line("=======================================================")
    print_line("【核心服务状态】")
...
def run_service_cmd(cmd):
    if shutil.which("systemctl"):
        subprocess.run(["systemctl", cmd, "nimbusvpn.service"])
    elif shutil.which("rc-service"):
        subprocess.run(["rc-service", "nimbusvpn", cmd])
    else:
        print("未检测到支持的服务管理器 (systemd/OpenRC)")

def start_service():
    print("正在启动 NimbusVPN 服务...", flush=True)
    run_service_cmd("start")
    print("已发送启动指令。")
    time.sleep(1)

def stop_service():
    print("正在停止 NimbusVPN 服务...", flush=True)
    run_service_cmd("stop")
    print("已发送停止指令。")
    time.sleep(1)

def restart_service():
    print("正在重启 NimbusVPN 服务...", flush=True)
    run_service_cmd("restart")
    print("已发送重启指令。")
    time.sleep(1)

def show_logs():
    print("正在查看 NimbusVPN 日志 (按 Ctrl+C 退出)...", flush=True)
...
def uninstall_service():
    confirm = input("确定要完全卸载 NimbusVPN 吗？(y/N): ")
    if confirm.lower() == 'y':
        print("正在完全卸载 NimbusVPN...", flush=True)
        stop_service()
        if shutil.which("systemctl"):
            subprocess.run(["systemctl", "disable", "nimbusvpn.service"])
            try:
                os.unlink("/lib/systemd/system/nimbusvpn.service")
            except Exception:
                pass
        elif shutil.which("rc-service"):
            subprocess.run(["rc-update", "del", "nimbusvpn"])
            try:
                os.unlink("/etc/init.d/nimbusvpn")
            except Exception:
                pass
        try:
...
def get_status_state():
    ...
    return (
        ...
        check_service_active("nimbusvpn.service"),
        check_openvpn_process(),
        get_service_pid("nimbusvpn.service")
    )
...
if [ -d "/etc/sysctl.d" ]; then
    cat > /etc/sysctl.d/99-nimbusvpn.conf <<EOF
net.ipv4.conf.all.rp_filter = 2
net.ipv4.conf.default.rp_filter = 2
EOF
    sysctl -p /etc/sysctl.d/99-nimbusvpn.conf >/dev/null 2>&1 || true
else
...
echo -e "\n正在启动 NimbusVPN 服务并初始化网络..."
if command -v systemctl >/dev/null 2>&1; then
    systemctl restart nimbusvpn.service || true
elif command -v rc-service >/dev/null 2>&1; then
    rc-service nimbusvpn restart || true
fi

# Wait and poll for node loading and active connection
echo -e "\n正在等待 NimbusVPN 首次获取节点并建立加密通道 (此过程可能需要 5-30 秒)..."
ACTIVE_ID=""
LAST_MSG=""
for i in {1..90}; do
...
echo -e "\n${GREEN}==========================================================${PLAIN}"
echo -e "${GREEN}             NimbusVPN 源码一键部署已完成！${PLAIN}"
echo -e "${GREEN}==========================================================${PLAIN}"
